...
{
	fire_lead (self, start, aimdir, 900, kick, TE_GUNSHOT, hspread, vspread, mod);
}
...
static void Grenade_Explode (edict_t *ent)
{
	...
	gi.WriteByte (svc_temp_entity);
	//This block of commented stuff should make it so that the GLauncher will always have the water effect when detonating.
	//if (ent->waterlevel)
	//{
		//if (ent->groundentity)
...
//CCH: Gonna be our new think function for homing missiles.
void homing_think(edict_t *ent)
{
	...
	if (target != NULL)
	{
		// target has been find, nudge our direction towards it.
...
void fire_rocket (edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, float damage_radius, int radius_damage)
{
	...
	//CCH: See if this is a player and if they have homing on. GAME MOD
